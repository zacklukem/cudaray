...
__host__ __device__ Image::Image(glm::vec3* data, size_t width, size_t height) : data(data), width(width), height(height) {}

__device__ Image::Image(size_t width, size_t height) : width(width), height(height) {
...
__host__ void Image::writeToFile(const char *filename) const {
  ...
  for (int i = 0; i < width * height; i++) {
    auto pixel = data[i];
    byte_data[i][0] = static_cast<uint8_t>(pixel.x * 255.0f + 0.5f);
    byte_data[i][1] = static_cast<uint8_t>(pixel.y * 255.0f + 0.5f);
    byte_data[i][2] = static_cast<uint8_t>(pixel.z * 255.0f + 0.5f);
  }
...
__device__ glm::vec2 Image::getUVCoord(const glm::ivec2 &pos, const glm::vec2 &rand) const {
